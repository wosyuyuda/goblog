...
# 模板使用
模板功能添加了多模板的选择功能，可以在配置里面配置模板路径，初始化时记得检查一下，新添加了手机端模板的模式，在原有的模板下方，添加一个m的模板目录即可，并且在配置里面添加是否开启手机端，如果是手机端访问，并且开启了手机模板，会自动切换到手机的模板  
新增加模板功能，可以在配置文件里面输入模板配置目录，并且在temp里面添加模板，模板名称必须为相应模板，不要随意修改，或者自己去改模板文件名称也行。

# 友情提醒  
golang这玩意真的不适合mvc结构,后台很多功能是前后端分离的...前台涉及到优化,用vue写不太好,还得换路由模式,太麻烦,就三个页面就先这样了,也懒得优化了

# 更新日志  
添加模板配置的功能，支持多个模板选择一个 2023-1-31  
添加文章 的评论功能 2022-3-8  
添加百度收录自动提交功能8.5  
支持输入代码
